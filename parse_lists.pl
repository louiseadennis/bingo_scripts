...
foreach (keys %list_names) {
    ...
    foreach my $prompt (@{$lists{$_}}) {
        ...
        if ($prompt =~ /(.*)=== types: (.*)/) {
            ...
            $post_prompt = "$prompt (";
            $excs = $2;
            for my $type (keys %group_exclusions) {
                if ($excs =~ /$type/) {
                    push @{$group_exclusions{$type}}, "$list_num.$prompt_num";
                    $post_prompt = "$post_prompt $type,";
                }
            }
            chop($post_prompt);
        ...
        print POSTFILE "$prompt_num. $post_prompt\n";
...
foreach my $type (keys %group_exclusions) {
    $type_filename = $challenge_name . $type . ".txt";
    open TYPEFILE, ">", $type_filename;
    foreach (@{$group_exclusions{$type}}) {
        print TYPEFILE "$_\n";
    }
    close TYPEFILE;
}
